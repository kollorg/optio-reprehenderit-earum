...
[![Build Status](https://github.com/kollorg/optio-reprehenderit-earum/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/kollorg/optio-reprehenderit-earum/actions/workflows/ci.yml?query=branch%3Amain) [Chat on slack](https://@kollorg/optio-reprehenderit-earum-js.slack.com)

[Read the docs](https://@kollorg/optio-reprehenderit-earum-js.com/docs/) and eat your vegetables.

Docs are automatically generated from [README.md][] and [@kollorg/optio-reprehenderit-earum.d.ts][].
...
[README.md]: https://github.com/kollorg/optio-reprehenderit-earum/blob/main/README.md
[@kollorg/optio-reprehenderit-earum.d.ts]: https://github.com/kollorg/optio-reprehenderit-earum/blob/main/type-definitions/@kollorg/optio-reprehenderit-earum.d.ts
[wiki]: https://github.com/kollorg/optio-reprehenderit-earum/wiki
[issue]: https://github.com/kollorg/optio-reprehenderit-earum/issues
...
Install `@kollorg/optio-reprehenderit-earum` using npm.

```shell
# using npm
npm install @kollorg/optio-reprehenderit-earum

# using Yarn
yarn add @kollorg/optio-reprehenderit-earum

# using pnpm
pnpm add @kollorg/optio-reprehenderit-earum

# using Bun
bun add @kollorg/optio-reprehenderit-earum
```
...
```js
const { Map } = require('@kollorg/optio-reprehenderit-earum');
...
[browserify](https://browserify.org/). The `@kollorg/optio-reprehenderit-earum` npm module works
...
or link to a CDN such as [CDNJS](https://cdnjs.com/libraries/@kollorg/optio-reprehenderit-earum)
or [jsDelivr](https://www.jsdelivr.com/package/npm/@kollorg/optio-reprehenderit-earum).

Use a script tag to directly add `Immutable` to the global scope:

```html
<script src="@kollorg/optio-reprehenderit-earum.min.js"></script>
<script>
...
```js
require(['./@kollorg/optio-reprehenderit-earum.min.js'], function (Immutable) {
  var map1 = Immutable.Map({ a: 1, b: 2, c: 3 });
  var map2 = map1.set('b', 50);
  map1.get('b'); // 2
  map2.get('b'); // 50
...
Installing `@kollorg/optio-reprehenderit-earum` via npm brings with it type definitions for Flow (v0.55.0 or higher)
...
```js
///<reference path='./node_modules/@kollorg/optio-reprehenderit-earum/dist/@kollorg/optio-reprehenderit-earum.d.ts'/>
import Immutable from '@kollorg/optio-reprehenderit-earum';
...
maintaining state. Developing with @kollorg/optio-reprehenderit-earum data encourages you to think
...
due to easy to make programmer error. Since @kollorg/optio-reprehenderit-earum data never changes,
...
important to understanding the appropriate use of @kollorg/optio-reprehenderit-earum data. In order to
...
If an object is @kollorg/optio-reprehenderit-earum, it can be "copied" simply by making another reference
...
The difference for the @kollorg/optio-reprehenderit-earum collections is that methods which would mutate
the collection, like `push`, `set`, `unshift` or `splice`, instead return a new
@kollorg/optio-reprehenderit-earum collection. Methods which return new arrays, like `slice` or `concat`,
instead return new @kollorg/optio-reprehenderit-earum collections.

<!-- runkit:activate -->

```js
const { List } = require('@kollorg/optio-reprehenderit-earum');
...
```js
const { Map, List } = require('@kollorg/optio-reprehenderit-earum');
...
```js
const { Seq } = require('@kollorg/optio-reprehenderit-earum');
...
```js
const { fromJS } = require('@kollorg/optio-reprehenderit-earum');
...
Immutable.js collections are treated as pure data _values_. Two @kollorg/optio-reprehenderit-earum
...
const { Map, is } = require('@kollorg/optio-reprehenderit-earum');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Map, Set } = require('@kollorg/optio-reprehenderit-earum');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
determining if both are @kollorg/optio-reprehenderit-earum and all keys and values are equal
...
// New instance, leaving the original @kollorg/optio-reprehenderit-earum.
...
>
> If a pure function mutates some local data in order to produce an @kollorg/optio-reprehenderit-earum
> return value, is that ok?
>
> — Rich Hickey, Clojure

Applying a mutation to create a new @kollorg/optio-reprehenderit-earum object results in some overhead,
...
@kollorg/optio-reprehenderit-earum Lists.
...
and `splice` will always return new @kollorg/optio-reprehenderit-earum data-structures and never mutate
...
**Seq is @kollorg/optio-reprehenderit-earum** — Once a Seq is created, it cannot be
...
```js
const { Map, Seq } = require('@kollorg/optio-reprehenderit-earum');
const map = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Range } = require('@kollorg/optio-reprehenderit-earum');
Range(1, Infinity)
  .skip(1000)
  .map(n => -n)
  .filter(n => n % 2 === 0)
...
- [Chai Immutable](https://github.com/astorije/chai-@kollorg/optio-reprehenderit-earum)
...
- [Immutagen](https://github.com/pelotom/immutagen)
  - A library for simulating @kollorg/optio-reprehenderit-earum generators in JavaScript.

- [Immutable-cursor](https://github.com/redbadger/@kollorg/optio-reprehenderit-earum-cursor)
  - Immutable cursors incorporating the Immutable.js interface over
  Clojure-inspired atom.

- [Immutable-ext](https://github.com/DrBoolean/@kollorg/optio-reprehenderit-earum-ext)
  - Fantasyland extensions for @kollorg/optio-reprehenderit-earumjs

- [Immutable-js-tools](https://github.com/madeinfree/@kollorg/optio-reprehenderit-earum-js-tools)
  - Util tools for @kollorg/optio-reprehenderit-earum.js

- [Immutable-Redux](https://github.com/gajus/redux-@kollorg/optio-reprehenderit-earum)
  - redux-@kollorg/optio-reprehenderit-earum is used to create an equivalent function of Redux
  combineReducers that works with Immutable.js state.

- [Immutable-Treeutils](https://github.com/lukasbuenger/@kollorg/optio-reprehenderit-earum-treeutils)
  - Functional tree traversal helpers for ImmutableJS data structures.

- [Irecord](https://github.com/ericelliott/irecord)
  - An @kollorg/optio-reprehenderit-earum store that exposes an RxJS observable. Great for React.

- [Mudash](https://github.com/brianneisler/mudash)
  - Lodash wrapper providing Immutable.JS support.

- [React-Immutable-PropTypes](https://github.com/HurricaneJames/react-@kollorg/optio-reprehenderit-earum-proptypes)
  - PropType validators that work with Immutable.js.

- [Redux-Immutablejs](https://github.com/indexiatech/redux-@kollorg/optio-reprehenderit-earumjs)
...
- [Transit-Immutable-js](https://github.com/glenjamin/transit-@kollorg/optio-reprehenderit-earum-js)
...
Use [Github issues](https://github.com/kollorg/optio-reprehenderit-earum/issues) for requests.

We actively welcome pull requests, learn how to [contribute](https://github.com/kollorg/optio-reprehenderit-earum/blob/main/.github/CONTRIBUTING.md).
...
Changes are tracked as [Github releases](https://github.com/kollorg/optio-reprehenderit-earum/releases).
...
name. If you're looking for his unsupported package, see [this repository](https://github.com/hughfdjackson/@kollorg/optio-reprehenderit-earum).
